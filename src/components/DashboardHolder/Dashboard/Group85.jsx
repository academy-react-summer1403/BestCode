import {Fragment} from 'react'
import images from '../../../assets/dashboardpng'

const Group85 = () => {
  return (
    <Fragment>
        <div className='w-[797px] h-[210px] border border-[#f7f7f7] dark:border-gray-800 mt-[30px] relative box-content' >
            <div className='h-[50px] w-[50px] absolute left-[41px] dark:bg-gray-800 bg-[]
              shadow-[0px_1px_2px_0px_#00000040] flex justify-center items-center rounded-[10px]
              bg-[#f7f7f7]
            '>
             <svg xmlns="http://www.w3.org/2000/svg" width="25" height="26" viewBox="0 0 25 26" fill="none">
                 <path d="M15.8943 19.7789C18.379 19.4848 20.8202 18.8984 23.1675 18.032C21.1797 15.8301 20.0813 12.9679 20.0857 10.0014V9.00125C20.0857 6.87919 19.2427 4.84404 17.7421 3.34351C16.2416 1.84299 14.2065 1 12.0844 1C9.96234 1 7.92719 1.84299 6.42666 3.34351C4.92614 4.84404 4.08315 6.87919 4.08315 9.00125V10.0014C4.08717 12.968 2.98829 15.8303 1 18.032C3.31103 18.8855 5.74741 19.4789 8.27447 19.7789M15.8943 19.7789C13.3632 20.0792 10.8056 20.0792 8.27447 19.7789M15.8943 19.7789C16.0865 20.3789 16.1343 21.0157 16.0338 21.6375C15.9333 22.2594 15.6874 22.8488 15.3161 23.3576C14.9448 23.8665 14.4585 24.2804 13.8969 24.5658C13.3354 24.8512 12.7143 25 12.0844 25C11.4545 25 10.8334 24.8512 10.2719 24.5658C9.71029 24.2804 9.22405 23.8665 8.85272 23.3576C8.4814 22.8488 8.23548 22.2594 8.135 21.6375C8.03452 21.0157 8.0823 20.3789 8.27447 19.7789" stroke="#B5B5B5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </div>
              <div className='w-[797px] h-[185px] shadow-[0px_1px_3px_0px_#00000040] rounded-[10px] mt-[25px] pt-[15px]
                flex justify-center  
              '>
                 
                 <div className='grid w-[710px] justify-start
                   ml-[8px] justify-items-start   
                 ' 
                 style={{direction:'rtl'}}
                 > 
                    
                    
                         <p className='
                            text-[20px] h-[28px]
                            flex items-center
                            font-primaryMedium text-[#777777]'>جدید ترین اخبار و مقالات
                         </p> 
                        <div className='mb-[12px] grid  '>
                        <div className='flex justify-between text-[14px] w-[710px] font-primaryMedium 
                      text-[#555555] items-center    h-[38.34px]' >
                          <p className='h-[20px]' >دوره آموزش جامع از پایه تا پیشرفته  Next.js منتشر شد.

                            <span className='mr-[7px] text-[#DC0000]' >جدید</span>
                          </p>
                          <p  className='text-[13px] text-[#999999]' 
                              style={{direction:'ltr'}}
                          >۱۴۰۲ / ۱۱ / ۳۰</p>
                        </div>
                         <div className='flex justify-between text-[14px] w-[710px]
                                          font-primaryMedium 
                                           text-[#555555]
                                        items-center 
                                        border-t-[1px] border-dashed
                                        border-b-[1px] border-[#AAAAAA] 
                                        h-[38.34px]
                           '>
                         <p className='h-[20px] ' >تخفیف ویژه دوره ری اکت را از دست ندهید.</p>
                         <p style={{direction:'ltr'}} 
                          className='text-[13px] text-[#999999]'
                         >۱۴۰۲ / ۱۱ / ۲۵</p>
                         </div>
                         <div className='flex justify-between text-[14px] w-[710px]
                                    font-primaryMedium 
                                    text-[#555555]
                                    items-center 
                                    h-[38.34px]
                                    
                                   
                          '>
                           <p className='h-[20px] '>دوره آموزش  tailwind  به روز شد.</p>
                           <p className='text-[13px] text-[#999999]'     style={{direction:'ltr'}} >۱۴۰۲ / ۱۱ / ۲۳</p>
                         </div>
                         </div>
                 </div>
              </div>
         </div>
    </Fragment>
  )
}

export default Group85